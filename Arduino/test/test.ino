...
const char* QUANTITY_INPUT = "apiqty";
const char* PRICE_INPUT = "apiprice";
const char* CUSTOMERID_INPUT = "apicustomerId";
   ...
   server.on("/change", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    // GET input1 value on <ESP_IP>/sale?qty=<qty>&price=<price>&customerId=<customerId>
     if (request->hasParam(QUANTITY_INPUT) && request->hasParam(PRICE_INPUT) && request->hasParam(CUSTOMERID_INPUT)) {
      apiqty = request->getParam(QUANTITY_INPUT)->value().toInt();
      apiprice = request->getParam(PRICE_INPUT)->value().toInt();
      apicustomerId = request->getParam(CUSTOMERID_INPUT)->value();
      apiFlag = true;
    }
