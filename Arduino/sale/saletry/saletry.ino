...
s.customerId = "";
      ...
      if (qtyFlag)
      {
        s.price = ceil((userInput * milkPrice) / 1000);
        s.qty = userInput;
        qtyFlag = false;
      }
      else
      {
        s.price = userInput;
        s.qty = ceil((userInput * 1000) / milkPrice);
      }
      Serial.println(s.price);
      Serial.println(s.qty);
      if (s.price > 0 && s.price < 400)
      {
        Serial.println(s.customerId);
        Serial.println(milkPrice);
        if (validateCustomer(s.customerId, s.price))
        {
          addSale(qty, price, customerId);
        }
      }
